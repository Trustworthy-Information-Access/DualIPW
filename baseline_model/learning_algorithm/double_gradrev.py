...
class GRL(torch.autograd.Function):
    ...
    def backward(self,grad_output):
        return grad_output.neg()*config.eta

class Gradrev_DNN(BaseAlgorithm):
    ...
    * Qingyao Ai, Keping Bi, Cheng Luo, Jiafeng Guo, W. Bruce Croft. 2018. Unbiased Learning to Rank with Unbiased Propensity Estimation. In Proceedings of SIGIR '18
    ...
    def __init__(self, exp_settings, ranking_model):
        ...
        if self.hparams.propensity_learning_rate < 0:
            self.propensity_learning_rate = float(self.hparams.learning_rate)*10
        else:
    ...
    def separate_gradient_update(self):
        ...
        if config.loss_mode!='pointwise':
            ...
            self.loss = self.loss + self.exam_loss + self.hparams.ranker_loss_weight * self.rank_loss
    ...
    def train(self, input_feed, epoch=0):
        ...
        # grl task
        self.rev_input=GRL.apply(self.propensity)
        self.rev_loss=nn.MSELoss(reduction='mean')(self.rev_input,train_labels)
        print(self.rev_loss.item())
        self.loss=self.rev_loss
        ...
        if config.loss_mode=='listwise':
            ...
            self.loss=self.loss+F.binary_cross_entropy_with_logits(p_c,train_labels,reduction='sum')/bs
